...
/*----------------------------------
  Product Type 3
 -----------------------------------*/
if(!function_exists('blonwe_product_type3')){  
	function blonwe_product_type3($stockprogressbar = '', $stockstatus = '', $shippingclass = '', $countdown = '', $product_sku = '', $productattributes = '', $product_brands = ''){
		global $product;
		global $post;
		global $woocommerce;
		...
		$postview  = isset( $_POST['shop_view'] ) ? $_POST['shop_view'] : '';
			...
			$output .= '<div class="product-wrapper style-3 with-content-fade product-type-3">';
			...
			$output .= blonwe_loop_add_to_cart($id,'product_type3');
			
			$output .= '</div><!-- product-cart-wrapper -->';
			
			ob_start();
			do_action('blonwe_product_box_footer', $stockprogressbar, $stockstatus, $shippingclass, $countdown, $product_sku, $productattributes, $product_brands);
			$output .= ob_get_clean();
